define(['knockout', 'jquery', 'text!./plp-inspector.html', 'appConfig', 'd3'], function (ko, $, view, appConfig, d3) {
        ...
        self.init = function () {
            ...
            d3.csv("./js/data/plp/" + self.modelId + "_inspector.csv", function (error, dataset) { // NEW
              ...
              'TestCovariateStDevWithOutcome', 'TestCovariateStDevWithNoOutcome', ]);
                ...
                table_plot.on('select', function (data, on_off) {
                    ...
                    if (on_off) { //if the data is highlighted
                        ...
                        d3.select("body").append("div")
                            ...
                            .style("top", (y(data.CovariateMeanWithOutcome) + 120) + "px");
                ...
                var r = d3.scale.linear().domain([0, maxVal]).range([1, 10]);
                ...
                var c = d3.scale.ordinal().range(["#8B0000", "#228B22"]).domain([-vMax, vMax]);
                ...
                var cFreq = d3.scale.linear().range([1, 10]).domain([0, 1]);
                var cFreqC = d3.scale.quantize().range(['black', 'green', 'green']).domain([0, 1]);
