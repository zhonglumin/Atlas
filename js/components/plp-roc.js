define(['knockout', 'jquery', 'text!./plp-roc.html', 'appConfig', 'd3', 'd3-slider'], function (ko, $, view, appConfig, d3, d3slider) {
    ...
    function plpRoc(params) {
        ...
        d3.csv("./js/data/plp/" + self.modelId + "_roc.csv", function (error, dataset) { // NEW
            ...
            svgContainerDist.selectAll(".bar1")
                ...
                .attr("y", function (d) {
                    return height - y2(maxheight - Math.abs(d.height1));
                })
                .attr("width", function (d) {
                    return x2(d.width)
                })
                .attr("height", function (d) {
                    return y2(maxheight - Math.abs(d.height1));
                });
            svgContainerDist.selectAll(".bar2")
                ...
                .attr("y", function (d) {
                    return height - y2(maxheight - Math.abs(d.height2));;
                })
                .attr("width", function (d) {
                    return x2(d.width)
                })
            ...
            var sensRange = d3.scale.quantize()
            ...
            var mySliderSens = d3slider()
                .min(100)
                .max(0)
                .axis(d3.svg.axis().ticks(1).tickFormat(tickFormatterSens))
            ...
            var mySliderSpec = d3slider()
                .min(0)
                .max(100)
                .axis(d3.svg.axis().ticks(1).tickFormat(tickFormatterSpec))
                .value(50)
                .on("slide", function (evt, value) {
